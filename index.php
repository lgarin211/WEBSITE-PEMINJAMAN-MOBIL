  ...
  <body>
    ...
    <div class="container mt-4">
      <div class="row">
        ...
        <div class="col-md-4">
          <div class="card">
            <img src="<?=$row['link_gambar']?>" class="card-img-top" alt="Gambar Mobil <?=$row['id']?>">
            <div class="card-body">
              <h5 class="card-title"><?=$row['jenis_mobil']."-".$row['kondisi']?></h5>
              <p class="card-text">Harga: <?=$row['harga']?></p>
              <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#CosMol<?=$row['id']?>">Pesan Sekarang</button>
            </div>
          </div>
        </div>
        <?php endwhile; ?>
    ...
    ?>
    <div class="modal fade" id="CosMol<?=$row['id']?>" tabindex="-1" aria-labelledby="exampleModalLabel<?=$row['id']?> " aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form action="http://localhost/WEBSITE-PEMINJAMAN-MOBIL/sewa.php" enctype="multipart/form-data" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memesan mobil ini?')" >
          <input type="hidden" name="idcard" value="<?=$row['id']?>">
          <div class="modal-body">
              <div class="form-group">
                <label for="nama">Nama</label>
                <input type="text" class="form-control" id="nama" name="nama_peminjam" placeholder="Masukkan Nama">
              </div>
              <div class="form-group">
    ...
    <script>
      function hitungTotalHarga() {
        ...
        var durasiPinjam = Math.floor((tanggalKembali - tanggalPinjam) / (1000 * 60 * 60 * 24)); // Menghitung durasi pinjam dalam hari
